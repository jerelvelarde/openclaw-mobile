...
36. **Canvas: tree-of-nodes vs HTML/WebView** `[mobile]` `[desktop]` `[protocol]` — surfaced by P10.0.
    ...
    - **Update (Wave 15):** P10A/B are retired in favor of the clawg-ui pivot. The Canvas tree-vs-WebView decision now lives in question #44 / `.chalk/plans/P11C-canvas-voice-realmode-strategy.md` — P11C recommends stub-only Canvas for v1 (Option C) with `@openclaw/canvas-ui` sibling plugin as the post-v1 path (Option B).
...
37. **Node-as-device callback surface** `[mobile]` `[desktop]` `[protocol]` — surfaced by P10.0.
    ...
    - **Update (Wave 15):** clawg-ui doesn't expose `node.invoke.request` callbacks either (it's a one-shot HTTP/SSE channel, not a persistent WS). Once we stop going through the P10A bridge, mobile becomes "AG-UI client only" — it cannot respond to gateway-initiated commands at all. That's an acceptable v1 stance (no feature on the roadmap needs it). If it becomes necessary post-v1, the right home is either a sibling plugin (per question #44 Option B) or a thin direct-WS channel alongside clawg-ui's HTTP. Defer.
...
38. **Adopt `HelloOk.features` capability discovery** `[mobile]` `[desktop]` `[protocol]` — surfaced by P10.0.
    ...
    - **Update (Wave 15):** clawg-ui doesn't surface `HelloOk.features` to clients — the only thing clients learn about gateway capabilities is implicit (the `/v1/clawg-ui` route either exists or 404s; an agent either responds or doesn't). For the v1 "hide Canvas/Voice tabs in real mode" use case in P11C, we hard-code from `gateway_mode === "clawg-ui"` instead of probing. Revisit post-v1 if Phase 1 (P11C Option B) sibling plugins ship and we need per-surface capability discovery.
...
39. **`setActiveAgent` doesn't map to anything upstream** `[mobile]` `[desktop]` `[protocol]` — surfaced by P10.0.
    ...
    - **Update (Wave 15):** clawg-ui uses an `X-OpenClaw-Agent-Id` request header per call (`vendor/clawg-ui/README.md:328–337`, defaults to `"main"`), which is the cleanest mapping of "pick agent per thread". P11A's URL builder includes the header; the renderer keeps the Agents tab as a picker for "default agent for new threads". `setActiveAgent` in `GatewayClient` becomes a no-op stored-locally setting (mobile-only) — kept for renderer ergonomics, not a wire concept.
...
40. **Pairing UX — keep our 6-digit code or adopt upstream's QR/setup-link?** `[mobile]` `[desktop]` — surfaced by P10.0.
    ...
    - **Update (Wave 15):** the bridge is being retired (P11D), so this is now a two-layer story. Layer 1 (unchanged): mobile pairs to the desktop via our 6-digit flow (P03B). Layer 2 (new, P11B): the desktop separately pairs to the user's openclaw daemon via clawg-ui's 403 `pairing_pending` flow, wrapped in a tray + Settings Approve button. The PR-back idea is moot for v1.

41. **`unsupportedInRealMode` error surfacing in apps** `[mobile]` `[desktop]` — surfaced by P10A. **SUPERSEDED by Wave 15 (clawg-ui pivot).**
    ...
    - **Action (original):** decide whether to land a thin "feature unsupported" banner in mobile (`apps/mobile/src/...`) and the desktop renderer that subscribes to `canvas.error` / `voice.error` and surfaces a Snackbar/Toast. Out of scope for P10A (chat-only happy path). Tracked here so P10C's e2e plan picks it up before we cut a real-mode build for real users.
    - **Update (Wave 15):** the P10A bridge is being torn down in P11D. The `unsupportedInRealMode` envelope ceases to exist with it. The post-pivot equivalent — "Canvas/Voice are stub-only in real mode" — is handled in P11A's renderer Settings change (hide the tabs) and documented in P11C. Resolved.

42. **clawg-ui pairing UX seam** `[desktop]` `[mobile]` — surfaced by Wave 15 pivot.
    - clawg-ui's pairing flow (`vendor/clawg-ui/README.md:209–315`) requires the gateway owner to run `openclaw pairing approve clawg-ui <code>` on the host CLI after the client gets a `403 pairing_pending`. That's terminal-only by default and breaks the "click one button" UX goal — the user just paired their phone via our 6-digit flow (P03B) and shouldn't have to also open Terminal.app to approve a second pairing they didn't ask for.
    - **Action (P11B):** wrap the CLI in a desktop Approve button + tray notification. The bigger open question is whether we should also propose a programmatic approve API to clawg-ui upstream (so the desktop doesn't have to shell out). For v1 the spawn is fine; long-term, an IPC/HTTP approve endpoint on clawg-ui itself would be cleaner. Track via a clawg-ui issue when P11B lands.

43. **clawg-ui plugin install path — desktop-managed or user-managed?** `[desktop]` — surfaced by Wave 15 pivot.
    - clawg-ui is a gateway-side plugin; the user installs it via `openclaw plugins install @contextableai/clawg-ui` against their daemon (`vendor/clawg-ui/README.md:24–37`). Today we treat that as a manual one-time step that lives in onboarding docs. Options if we want to automate it: (a) desktop detects the absence of clawg-ui (the `/v1/clawg-ui` route 404s) on first real-mode connect and offers a "Install plugin" button that spawns `openclaw plugins install ...` — same spawn pattern as P11B's approve. (b) document only; rely on the README. (c) bundle the clawg-ui dist into the desktop and copy it into the user's openclaw extensions dir on first boot (more invasive, breaks the "user owns their gateway" trust model).
    - **Action:** decide before v1 real-mode launch. Recommended: option (b) for v1, option (a) post-v1 once P11B's spawn pattern is proven. Tracked here so the v1 release plan adds a "before flipping to real mode, install clawg-ui on your gateway" line to onboarding.

44. **Canvas + Voice real-mode strategy** `[mobile]` `[desktop]` `[protocol]` — surfaced by Wave 15 pivot.
    - clawg-ui is chat-only (`vendor/clawg-ui/src/channel.ts:20–23`, `chatTypes: ["direct"]`). Our P06 (Canvas) and P07 (Voice) surfaces have no real-mode path through it. Three options enumerated in `.chalk/plans/P11C-canvas-voice-realmode-strategy.md`: (A) PR Canvas/Voice into clawg-ui (fork the vendored repo); (B) sibling plugins `@openclaw/canvas-ui` + `@openclaw/voice-ui`; (C) stub-only for v1.
    - **Action:** P11C recommends C for v1, B (staged Canvas first, Voice later) for post-v1. Triggers for Phase 1 (Canvas) and Phase 2 (Voice) documented in P11C. Re-evaluate when the first TestFlight user asks for real-mode Canvas.

45. **Fork-vs-vendor revisit trigger** `[root]` — surfaced by Wave 15 pivot.
...
46. **Where does the clawg-ui base URL come from on the mobile side?** `[mobile]` `[desktop]` — surfaced by P11A. **RESOLVED in Wave 15 review.**
    - P11A wires the mobile chat client to POST `<clawgUiBaseUrl>/v1/clawg-ui` when running in `mode: "clawg-ui"`. For the v1 happy path (one openclaw daemon per user, co-located with the desktop) the natural source is "whatever the desktop advertises at pairing time", but P11A did **not** add a new field to `PairingApproved` — that was deferred to a P11B follow-up.
    - **Resolution:** `PairingApproved` now carries an optional `clawgUiBaseUrl` field (`packages/protocol/src/types.ts` + `schemas.ts`). The desktop pair server populates it from `http://<gatewayHost>:<gatewayPort>` when `gateway_mode === "clawg-ui"` (`apps/desktop/src/main/pair/server.ts` + `controller.ts`). The mobile pairing reducer + secure-store session persist it alongside `runtimeUrl` / `httpBase` (`apps/mobile/src/pairing/{state,store}.ts`), and `apps/mobile/app/_layout.tsx` threads it into `CopilotKitProvider({ mode: 'clawg-ui', runtimeUrl: clawgUiBaseUrl, ... })` so chat in real mode hits `<baseUrl>/v1/clawg-ui` automatically. Optional in the Zod schema so older desktop builds (and stub mode) keep validating.

47. **clawg-ui operator-auth route as a per-device-pairing bypass** `[desktop]` — surfaced by P11B.
...
48. **`openclaw` npm package availability for the e2e Docker image** `[root]` — surfaced by P11E. **Reproducibility narrowed; availability still pending.**
    - `e2e/docker/Dockerfile.openclaw-gateway` does
      `RUN npm install -g openclaw@${OPENCLAW_VERSION}` with
      `ARG OPENCLAW_VERSION=latest` (Wave 15 review). The pin is now
      explicit, so once the upstream project tags a stable release we
      can bump the floor and the nightly image becomes byte-deterministic
      against that tag — see `e2e/README.md` "Bumping the openclaw pin".
      We still cannot independently confirm against the public npm
      registry that `openclaw` is published under that exact name (vs. e.g.
      `@openclaw/cli` or a GitHub-release-only distribution). The dev
      container this work was authored in cannot reach the internet, so
      the only test of the install path is the CI workflow itself.
    - **Status:** pin syntax resolved. Live availability stays open until
      the first nightly `.github/workflows/e2e-clawg-ui.yml` run completes
      successfully.
    - **Action:** first nightly will either succeed (resolve the question) or fail loudly at the `npm install -g openclaw@${OPENCLAW_VERSION}` step. If it fails, options are: (a) switch to `RUN git clone https://github.com/openclaw/openclaw && cd openclaw && npm ci && npm run build && npm link` in the Dockerfile (slower but doesn't depend on a published artifact); (b) vendor the openclaw dist alongside `vendor/clawg-ui/`; (c) wait for the upstream project to publish to npm if they haven't already. Tracked here because the harness skeleton is correct either way — only the one RUN line changes.
