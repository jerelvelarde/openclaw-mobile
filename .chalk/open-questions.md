...
46. **Where does the clawg-ui base URL come from on the mobile side?** `[mobile]` `[desktop]` — surfaced by P11A.
    - P11A wires the mobile chat client to POST `<clawgUiBaseUrl>/v1/clawg-ui` when running in `mode: "clawg-ui"`. For the v1 happy path (one openclaw daemon per user, co-located with the desktop) the natural source is "whatever the desktop advertises at pairing time", but P11A does **not** add a new field to `PairingApproved` — that's P11B's job (extend the pairing payload with a `clawgUiBaseUrl` + `clawgUiDeviceToken` pair sourced from the desktop's identity store).
    - **Open question:** should the mobile derive the URL from the same `httpBase` it paired against (treating the daemon as always-on-the-same-host as the desktop), or should it be a separate field so the gateway can live on a different host than the desktop? The Mac-mini-as-daemon-host assumption (`.chalk/open-questions.md` §C "phone is always a client of a user-hosted gateway, fronted by openclaw-desktop") implies the first; a future "desktop on laptop, gateway on home server" deployment implies the second.
    - **Action (P11B follow-up):** add an explicit `clawg_ui_base_url` field to `PairingApproved` rather than deriving it. Default it to `<httpBase host>:18789` (matching today's deployment) but persist it as a discrete value so users running a split topology can override without rebuilding the mobile app. Cross-references: `apps/mobile/src/copilot/runtimeUrl.ts#resolveRuntimeRequest` already accepts `clawgUiBaseUrl` as a separate option, so the plumbing is in place.
