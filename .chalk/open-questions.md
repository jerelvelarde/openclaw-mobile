...
32. **Desktop stub gateway needs `voice.open` / `voice.close` / `voice.signal` / `voice.transcript` topics + WebRTC peer** `[desktop]` `[mobile]`
    - P07A landed the mobile voice surface end-to-end: `voice.tsx` PTT screen, `usePushToTalk` FSM, mic permissions, and a `RealGateway.openVoice` / `sendVoiceSignal` / `onVoiceSignal` / `onVoiceTranscript` quartet that posts `voice.open` + relays `voice.<sessionId>.signal` + fans out `voice.<sessionId>.transcript`. The desktop stub gateway (`apps/desktop/src/main/gateway/stub.ts`) doesn't yet handle any of these: a real round-trip needs (a) `voice.open` → spawn an agent-side peer + reply `voice.open.response`, (b) `voice.signal` relay (mobile offer → desktop peer SDP answer → trickle ICE both ways), (c) `voice.transcript` broadcasts from the agent during/after a turn, (d) `voice.close` teardown. None of this exists in the stub yet; the mobile screen will sit in `arming` forever against today's desktop build. P07B is scoped for the **desktop client** voice surface (PTT in the Electron window); a separate plan needs to land the agent-side WebRTC peer + transcript emitter to make either client functional end-to-end. Until then, the mobile module is exercised only by its unit tests + the mocked-gateway signaling round-trip. Codec choice for v1 is pinned at **opus @ 16kHz mono** (`VoiceOpts { format: "opus", sampleRate: 16000 }`) — both clients should keep that constant in lockstep until a real hardware test prompts re-tuning.
