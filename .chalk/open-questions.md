...
24. **Committed native projects from `expo prebuild`** `[mobile]` `[process]`
...
25. **Mobile WS protocol stubs await P04B** `[mobile]` `[desktop]`
    - P04A's `RealGateway` implements the HTTP pairing surface end-to-end against `apps/desktop`'s `127.0.0.1:18789` server but leaves `listAgents` / `listThreads` / `postMessage` / `streamThread` as TODO stubs (return `[]` / throw a typed "P04B" error) because the desktop side hasn't shipped the `agents.*` / `threads.*` WS topics yet. The reconnect controller, heartbeat, envelope codec, and event dispatch are all live and unit-tested — they just have no real frames to carry. Once P04B lands the WS endpoint we need to (a) define the request/response envelope types in `@openclaw/protocol`, (b) wire `RealGateway` to send/await them, and (c) replace the TODO returns with real implementations. The WS close-code range `4001-4099` is reserved for `token_expired` per `RealGateway`'s `onClose` handler — confirm with P04B's WS implementation before promising that behavior.
